...
import { clearSnapshots, exportBackup, importBackup, listSnapshots, restoreSnapshot, snapshotCount, snapshotNow } from "../../services/backup";
import { requestRefresh } from "../../app/bus";
import { $, showToast } from "../dom";
import { testApiKey } from "../../services/llm/index";
...
export function settingsFromForm(): Settings {
  return {
    ...
    excludePrivateHosts: ($("excludePrivateHostsInput") as HTMLInputElement).checked,
...
export function openSetup(asOptions: boolean): void {
  ...
  void getSettings().then((s) => {
    ...
    ($("excludePrivateHostsInput") as HTMLInputElement).checked = s.excludePrivateHosts;
