...
- **The AI can ask, only you can send** — when a request needs bookmarks the model cannot see ("move my old articles under Research"), it asks for the library. The Before-sending step then appears no matter what, says why it was asked, and Send or Cancel is yours; a shared library stays shared for that conversation only.
